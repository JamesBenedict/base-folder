...
All project folders must be less than 80MB, so size down all your media to only what's needed for the web. There is a lot of extra javascript in the template as it shows you every option available. Try to keep as much media in your folder as possible, but if you need to use an external service to host it use an official IDS news account.</p>
